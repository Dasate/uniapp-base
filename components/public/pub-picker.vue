<!-- 选择器 -->
<!-- 二改于u-picker 增加返回源数据，@initData方法 -->
<template>
  <u-popup
...
export default {
  name: 'u-picker',
  emits: ['update:modelValue', 'input', 'confirm', 'cancel', 'close','initData'],
  props: {
    // 通过双向绑定控制组件的弹出与收起
    value: {
      type: Boolean,
  ...
  methods: {
    ...
    init() {
      ...
      if(this.isInit && this.item.default){
          let __data = {};
          if (this.mode == 'time') {
            if (this.item._params.year) __data.year = this.formatNumber(this.year || 0);
            if (this.item._params.month) __data.month = this.formatNumber(this.month || 0);
            if (this.item._params.day) __data.day = this.formatNumber(this.day || 0);
            if (this.item._params.hour) __data.hour = this.formatNumber(this.hour || 0);
            if (this.item._params.minute) __data.minute = this.formatNumber(this.minute || 0);
            if (this.item._params.second) __data.second = this.formatNumber(this.second || 0);
          } else if (this.mode == 'region') {
            if (this.item._params.province) __data.province = provinces[this.province];
            if (this.item._params.city) __data.city = citys[this.province][this.city];
            if (this.item._params.area) __data.area = areas[this.province][this.city][this.area];
          }
          this.$emit('initData',__data,this.item);
        }
      this.$forceUpdate();
    },
    // 设置picker的某一列值
    ...
    getResult(event = null) {
      ...
      if (event) this.$emit(event, result, this.item);
      this.close();
    },
    // 获取时间戳
    getTimestamp() {
